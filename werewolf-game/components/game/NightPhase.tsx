...
export default function NightPhase({ state, socket, messages }: Props) {
  ...
  const aliveWolfCount = state.players.filter(p => p.role === 'werewolf' && p.isAlive).length
  const isArenaMultiWolf = state.gameMode === 'arena' && isWerewolf && aliveWolfCount > 1
  const isArenaSoloWolf = state.gameMode === 'arena' && isWerewolf && aliveWolfCount === 1
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 h-full p-4 max-w-5xl mx-auto w-full">
      <div className="flex-1 flex flex-col gap-4">
        ...
        {/* Arena multi-wolf panel */}
        {canAct && isArenaMultiWolf && (
          <WerewolfArenaPanel state={state} socket={socket} />
        )}

        {/* Werewolf (classic or arena solo) / Seer action panel */}
        {canAct && !isArenaMultiWolf && (role === 'werewolf' || role === 'seer') && (
              ...
              <button
                ...
                className={`text-sm px-3 py-2 rounded-lg border transition-colors text-left cursor-pointer ${
                  selected === WOLF_VOTE_NOBODY
                    ...
                    : 'border-slate-700 bg-slate-800 text-slate-300 hover:border-amber-700'
                }`}
              >
      ...
      {isWerewolf && isAlive && state.gameMode === 'classic' && (
