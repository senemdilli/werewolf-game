...
| `--retry-on-self-label` | `flag` | Optional. If set, rejects tool call, and requests LLM retry if the model accidentally includes itself in `report_labels` output (default: `false`) |
...
| `get_labels(phase_idx)` | Returns human labels as `observer -> target -> labels`; see [Trust Dimensions](#trust-dimensions) for the label dimensions |
...
The parser converts these exported scores into immutable `Label`, `TrustScores`, and `Score` objects internally.
