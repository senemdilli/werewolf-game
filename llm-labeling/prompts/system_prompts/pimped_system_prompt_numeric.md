...
For each statement, assign exactly one rating using an integer from 1 (lowest trust) to 7 (highest trust).
 
Immediately after each statement rating, assign exactly one confidence rating using an integer from 1 (low confidence) to 3 (high confidence).
